...
function readOperationalID(req, res) {
    ...
    const query = connect.con.query('SELECT id_operational, name, name, birth_date, adress, entry_date, cc, phone_number, pay_per_hour, operational_type, speciality, id_login FROM operational where ? ', post, function(err, rows, fields) {
        console.log(query.sql);
        if (err) {
            console.log(err);
...
function saveOperational(req,res){
    ...
    if (idOperational != "NULL" && nameOperational != "NULL" && birthDate != "NULL"  && adressOperational != "NULL" && entryDate != "NULL" 
        && ccOperational != "NULL" && phoneNumberOperational != "NULL" && paymentOperational != "NULL" && operationalType != "NULL"
        && specialityOperational != "NULL" && loginOperational != "NULL" && typeof(idOperational) != 'undefined' && typeof(nameOperational) != 'undefined' 
...
function updateOperational(req, res) {
    ...
    if (errors) {
        ...
        && typeof(operationalType) != 'undefined' && typeof(specialityOperational) != 'undefined')
          {
            const update = [adressOperational, phoneNumberOperational, paymentOperational, operationalType, specialityOperational, idOperational];
            const query = connect.con.query('UPDATE operational SET adress =?, phone_number =?, pay_per_hour=?, operational_type=?, speciality=? WHERE id_operational=?', update, function(err, rows, fields) {
                console.log(query.sql);
                if (!err) {
